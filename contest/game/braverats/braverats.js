class BraveRats extends Game {
	...
	*/			
	constructor (context, uri, agent_1, agent_2, language, seed=0) {
		...
		super ("BraveRats", context, uri, agent_1, agent_2, seed, null, true, false)
		this._gui_player = -1
	...
	*/
	update (dt) {		
		...
		this._figures.forEach (f => {
			...
			// Player has to play ?
			if (this._actions.length > 0 && card.slot == -1 && this._message == null) {
		...
		if (this._message) {
			this._message.t += dt / 1000.0
			if (this._message.t > 2.0) this._message = null
		}
	}
	...
	*/
	draw () {
		...
		this._figures.forEach (f => {
			...
			if (card.is_busy ()) moving.push (card)
			else card.draw ()
			...
			// Draw help text for card that is highlighted
		...
		// Draw round scores above played cards
		this._draw_scores ()

		// Draw moving card above
		moving.forEach (card => {card.draw ()})

		// Draw message
		if (this._message != null) {
			var [x, y] = this.get_slot_coo (0, 1.75)
			this._draw_banner (this._message.msg, this._message.color, y, -1)
		}
	...
	/** ===========================================================================================
	* Return the coordinate of a card slot on the middle of the screen
	*/
	...
	*/
	_round_start (contest, round_number, game_state) {
		...
		this._message = {msg: `Round ${round_number +1}`, t: 0.0, color: "rgba(0,0,0,0.6)"}
	}
	...
	* @param {string} game_state	The current state of the game encoded into a string
	* @param {Objet}  player		Array of players that have to play now
	*/
	_turn_start (contest, round_number, step, game_state, player) {
		if (player.length == 1 && step == 0)
		{
			var colors = ["rgba(200, 0, 0, 0.6)", "rgba(0, 0, 200, 0.6)"]
			if (player [0].number == 0) 
				this._message = {msg: this._language.player_first, t: 0.0, color: colors [this._swap_color]}
			else
				this._message = {msg: this._language.other_first, t: 0.0, color: colors [1 - this._swap_color]}
		}
	}
	...
	*/
	_round_end (contest, game_state, rewards) {
		...
		// Detect suspended round
		var num_suspended = Number (game_tokens [2].split (':') [1])
		if (num_suspended > 0) {
			var keys = Object.keys (this._deck_blue)
			keys.forEach (k => {
				if (this._deck_blue [k].slot == this._round) this._deck_blue [k].set_scale (0.70)
			})

			var keys = Object.keys (this._deck_red)
			keys.forEach (k => {
				if (this._deck_red [k].slot == this._round) this._deck_red [k].set_scale (0.70)
			})
		}
	}
